    ...
    use ModelParams
    real(dl)  GetOmegak
! VS 8/5/19 2nd CDM species
    !GetOmegak = 1 - (CP%omegab+CP%omegac+CP%omegav+CP%omegan)
    GetOmegak = 1 - (CP%omegab+CP%omegac+CP%omegac2+CP%omegav+CP%omegan)
!
...
! VS 2019 - DCDM & DR background EOMs
    subroutine DCDM_DR_derivs(EV,n,lna,fy,fyprime)
    integer n
    real(dl) fy(n), fyprime(n)
    real(dl), intent(IN) :: a
    real(dl) lna
    ! Internal variables
    real(dl) OmegaR, OmegaM, Hub, MX, RX

    lna = log(a)
    OmegaR = CP%omegag + CP%omegan
    OmegaM = CP%omegab + CP%omegac

    Hub = sqrt((OmegaR*exp(-4.0*lna) + OmegaM*exp(-3.0*lna) + exp(MX) + exp(RX) + (1-OmegaM)))

    fy(1) = MX
    dMXdlna = -3.0_dl - exp(lna)*CP%GamDCDM/Hub
    fyprime(1) = dMXdlna

    fy(2) = RX
    dRXdlna = -4.0_dl + exp(lna)*CP%GamDCDM*exp(MX-RX)/Hub
    fyprime(2) = dRXdlna


    end subroutine DCDM_DR_derivs

    subroutine DCDM_DR_background
    ! Integrate background EOMs, make table then interpolate results

    real(dl)

    !dverk (EV,n, fcn, x, y, xend, tol, ind, c, nw, w)

    fy(1) = MXinit
    fy(2) = RXinit
    call dverk(EV,2,DCDM_DR_derivs,lna,fy,tauend,1e-6,ind,c,EV%nvar,w)

    end subroutine DCDM_DR_background
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !  8*pi*G*rho*a**4.
! VS 2019 2nd CDM species and DR
    !grhoa2=grhok*a2+(grhoc+grhob)*a+grhog+grhornomass
    grhoa2=grhok*a2+(grhoc+grhoc2+grhob)*a+grhog+grhornomass+grhodr
!
    if (w_lam == -1._dl) then
...
! VS 2019 2nd CDM species and DR
        integer d2_ix !Index of 2nd CDM species
        integer dr_ix !Index for dark radiation
!
...
! VS 2019 added for dark radiation
        integer lmaxdr, lmaxdrt, lmaxdrv
!
...
! VS 2019 dark radiation addition
        logical high_ktau_darkrad_approx
!
...
! VS 2019 dark radiaiton addition
        logical no_darkrad_multpoles
!
    ...
    ABSTRACT INTERFACE
!VS 8/5/19 2nd CDM species and DR
    SUBROUTINE TSource_func(sources, tau, a, adotoa, grho, gpres,w_lam, cs2_lam,  &
        ...
        tau0, tau_maxvis, Kf, f_K, grhoc2_t, clxc2, vc2 , grhodr_t, clxdr, qdr)
    ...
    real*8, intent(in) :: tau, a, adotoa, grho, gpres,w_lam, cs2_lam,  &
        ...
        opacity, dopacity, ddopacity, visibility, dvisibility, ddvisibility, exptau, &
        tau0, tau_maxvis, grhoc2_t, clxc2, vc2, grhodr_t, clxdr, qdr
!
    REAL*8, intent(in) :: Kf(*)
    real*8, external :: f_K
...
! VS 2019 dark radiation addition
    real(dl) tau_switch_no_nu_multpoles,  tau_switch_no_phot_multpoles, tau_switch_nu_nonrel, tau_switch_no_darkrad_multpoles
!
...
! VS 2019 dark radiation addition
    tau_switch_no_darkrad_multpoles = noSwitch
!
...
! VS 2019 dark radiation addition
    if (.not. EV%high_ktau_darkrad_approx .and. .not. EV%no_darkrad_multpoles ) then
...
! VS 2019 dark radiation addition
        if (.not. EV%no_darkrad_multpoles) &
            tau_switch_no_darkrad_multpoles=max(15/EV%k_buf*AccuracyBoost,min(taurend,matter_verydom_tau))
!
...
! VS 2019 added dark radiaiton
    next_switch = min(tau_switch_ktau, tau_switch_nu_massless,EV%TightSwitchoffTime, tau_switch_nu_massive, tau_switch_no_nu_multpoles, tau_switch_no_darkrad_multpoles, tau_switch_no_phot_multpoles, tau_switch_nu_nonrel,noSwitch)
!
...
!VS 2019 added for dr
            EVout%high_ktau_darkrad_approx=.true.
!
            ...
            EV=EVout
! VS 2019 dark radiation addition
        else if (next_switch==tau_switch_no_darkrad_multpoles) then
...
! VS 8/5/19 2nd CDM species
    ...
    maxeq = maxeq + 2
!
! VS 2019 dark radiaiton additon
    if (.not. EV%no_darkrad_multpoles) then
        !Massless neutrino multipoles
        EV%dr_ix= neq+1
...
! VS 2019 dark radiation addition
    if (.not. EV%no_darkrad_multpoles .and. .not. EVout%no_darkrad_multpoles) then
...
! VS 8/5/19 2nd CDM species
    yout(EVout%d2_ix) = y(EV%d2_ix)
    yout(EVout%d2_ix+1) = y(EV%d2_ix+1)
!
...
! VS 2019 dark radiation addition
    EV%high_ktau_darkrad_approx = .false.
!
    if (CP%WantScalars) then
        EV%TightCoupling=.true.
        EV%no_phot_multpoles =.false.
        EV%no_nu_multpoles =.false.
! VS 2019 dark radiation approximation
        EV%no_darkrad_multpoles =.false.
!
...
! VS 2019 dark radiation addition
        EV%lmaxdr = max(nint(14*lAccuracyBoost),3)
!

        if (max_nu_mass>700 .and. HighAccuracyDefault) EV%lmaxnr = max(nint(32*lAccuracyBoost),3) !Feb13 tweak
! VS 2019 dark radiation addition
        if (HighAccuracyDefault) EV%lmaxdr = max(nint(32*lAccuracyBoost),3)
!
...
! VS 2019 dark radiation addition
            EV%lmaxdr=max(3,nint(min(7,nint(sqrt(scal)* 150 * EV%q))*lAccuracyBoost))
!
...
! VS 2019 dark radiation addition
                EV%lmaxdr=max(nint(45*lAccuracyBoost),3)
!
            else
                EV%lmaxnr=max(nint(30*lAccuracyBoost),3)
! VS 2019 dark radiation addition
                EV%lmaxdr=max(nint(30*lAccuracyBoost),3)
!
            endif
...
! VS 2019 dark radiation addition
            EV%lmaxdr=min(EV%lmaxdr, EV%FirstZerolForBeta-1)
!
...
! VS 2019 dark radiation addition
        ...
        EV%MaxlNeeded=max(EV%lmaxg,EV%lmaxnr,EV%lmaxgpol,EV%lmaxnu,EV%lmaxdr)
!
...
! VS 2019 dark radiation addition
    ...
    om = (grhob+grhoc)/sqrt(3*(grhog+grhonu+grhodr))
!
    omtau=om*tau
!VS 2019 dark radiation addition
    !Rv=grhonu/(grhonu+grhog)
    Rv=grhonu/(grhonu+grhog+grhodr)
!
    Rg = 1-Rv
! VS 8/5/19 2nd CDM species
    !Rc=CP%omegac/(CP%omegac+CP%omegab)
    Rc=CP%omegac/(CP%omegac+CP%omegac2+CP%omegab)
    Rc2=CP%omegac2/(CP%omegac+CP%omegac2+CP%omegab)
    Rb=1-Rc-Rc2
!
    Rp15=4*Rv+15
    ...
    end if !VS 2019 suggested typo

! VS 8/5/19 2nd CDM species
    y(EV%d2_ix) = InitVec(i_clxc)
    y(EV%d2_ix+1) = 0.0
!

!  VS 2019 Dark Radiation addition
    y(EV%dr_ix)=InitVec(i_clxr)
...
! VS 8/5/19 no omegad for vectors
    Rc=CP%omegac/(CP%omegac+CP%omegab)
...
! VS 2019 dark radiation additions
    real(dl) clxdrdot, grhodr_t, qdrdot, pidrdot,clxdr, qdr, pidr
    integer jx
!
    k=EV%k_buf
    k2=EV%k2_buf
...
! VS 8/5/19 2nd CDM species
    ...
    dgrho_matter=grhob_t*clxb+grhoc_t*clxc+grhoc2_t*clxc2
!
    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
! VS 8/5/19 2nd CDM species
    dgq=grhob_t*vb+grhoc2_t*vc2
!
...
! VS 8/5/19 2nd CDM species
    ...
    grho_matter=grhonu_t+grhob_t+grhoc_t+grhoc2_t
    grho = grho_matter+grhor_t+grhog_t+grhov_t+grhodr_t
!
    if (CP%flat) then
        adotoa=sqrt(grho/3)
        cothxor=1._dl/tau
...
! VS 2019 dark radiation addition
    if (EV%no_darkrad_multpoles) then
        z=(0.5_dl*dgrho/k + etak)/adotoa
        dz= -adotoa*z - 0.5_dl*dgrho/k
        clxdr=-4*dz/k
        qdr=-4._dl/3*z
...
! VS 2019 dark radiation addition
    ...
    dgrho=dgrho + grhog_t*clxg+grhor_t*clxr+grhodr_t*clxdr
!

    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
! VS 2019 dark radiation addition
    !dgq=dgq + grhog_t*qg+grhor_t*qr
    dgq=dgq + grhog_t*qg+grhor_t*qr+grhodr_t*qdr
!
...
! VS 8/5/19 2nd CDM species
    ayprime(EV%d2_ix) = -k*z !-vc2
    ayprime(EV%d2_ix+1) = 0.0
!
...
! VS 2019 dark radiation addition
    if (.not. EV%no_darkrad_multpoles) then
        ...
        clxdrdot= -k*qdr - (4._dl/3._dl)*k*z + a*CP%GamDCDM*(grhoc2/grhodr)*(clxc2-clxdr)
        ayprime(EV%dr_ix)=clxdrdot
        qdrdot= (k/3._dl)*clxdr - (2._dl*k/3._dl)*pidr - a*CP%GamDCDM*(grhoc2/grhodr)*qdr
        ...
        if (EV%high_ktau_darkrad_approx) then
            ...
            jx=EV%dr_ix+2
            if (EV%lmaxdr>2) then
                pidrdot= EV%denlk(2)*qdr - EV%denlk2(2)*ay(jx+1) + 8._dl/15._dl*k*sigma - a*CP%GamDCDM*(grhoc2/grhodr)*pidr ! STANDARD CASE
                ayprime(jx)=pidrdot
                do l=3,EV%lmaxdr-1
                    jx=jx+1
                ...
                pidrdot= EV%denlk(2)*qdr + 8._dl/15._dl*k*sigma - a*CP%GamDCDM*(grhoc2/grhodr)*pidr !STANDARD - l=3 TERM
                ayprime(jx)=pidrdot
            end if
        end if
...
!VS 2019 dark radiation addition
        ...
        dgpi  = grhor_t*pir + grhog_t*pig + grhodr_t*pidr
!
        dgpi_diff = 0  !sum (3*p_nu -rho_nu)*pi_nu
! VS 2019 dark radiation addition
        !pidot_sum = grhog_t*pigdot + grhor_t*pirdot
        pidot_sum = grhog_t*pigdot + grhor_t*pirdot + grhodr_t*pidrdot
!
        clxnu =0
        if (CP%Num_Nu_Massive /= 0) then
...
! VS 2019 dark radiation addition
        !gpres=gpres_nu+ (grhog_t+grhor_t)/3 +grhov_t*w_lam
        gpres=gpres_nu+ (grhog_t+grhor_t+grhodr_t)/3 +grhov_t*w_lam
!
        ...
        if (associated(EV%OutputSources)) then
            ...
            if (associated(EV%CustomSources)) then
                call custom_sources_func(EV%CustomSources, tau, a, adotoa, grho, gpres,w_lam, cs2_lam, &
                    ...
                    polter, polterdot, polterddot, octg, octgdot, E, Edot, &
                    opacity, dopacity, ddopacity, visibility, dvisibility, ddvisibility, exptau, &
                    tau0, tau_maxvis, EV%Kf,f_K, grhoc2_t, clxc2, vc2 , grhodr_t, clxdr, qdr)
            end if
        end if
    end if
...
! VS 8/5/19 2nd CDM species
    !grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t
    grho=grhob_t+grhoc_t+grhoc2_t+grhor_t+grhog_t+grhov_t
!
